...
scenario_name = "fricrampcloth"
trial_name = "pilot_it2_rollingSliding_simple_ramp_box_2distinct_singleramp"
trial_id = 0
show_timestep = 80
...
if use_obi:
    ...
    obj_pts = []
    for idx in data['static']["obi_object_ids"].tolist():
        if str(idx) in particle_positions:
            pts = particle_positions[str(idx)] * 0.6
            colors = np.zeros((pts.shape[0], 4), dtype=np.uint8)
            colors[:,3] = 255
            colors[:,2] = 255
            obj_pts.append(trimesh.PointCloud(pts, colors=colors))

(trimesh.Scene(obj_meshes_pts[0]) + trimesh.Scene(obj_meshes_pts[1]) + trimesh.Scene(obj_meshes_pts[2]) + trimesh.Scene(obj_meshes_pts[3])+ trimesh.Scene(obj_pts[0]) + camera + axis).show()
