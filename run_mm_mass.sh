...
if [ "$HOSTNAME" = aw-m17-R2 ]; then
    ...
    DUMP_DIR="/mnt/fs4/hsiaoyut/physion++/mini_v4"
else
...
#     CUDA_VISIBLE_DEVIES=$1 python $controller @$args_f"/commandline_args.txt" --dir "$DUMP_DIR"${args_f#$ARGS_PATH}  --training_data_mode --height 128 --width 128 --gpu $1 --num 10 --seed $counter
# done
...
for args_f in $ARGS_PATH"/"$scenario_name/$sname-target=bowl-zdloc=1*
do
    echo "found file" $args_f
    echo "write data to" "$DUMP_DIR"${args_f#$ARGS_PATH}

    CUDA_VISIBLE_DEVIES=$1 python $controller @$args_f"/commandline_args.txt" --dir "$DUMP_DIR"${args_f#$ARGS_PATH}  --training_data_mode --height 128 --width 128 --gpu $1 --num 8
done


# sname="mass_collision"
# scenario_name=$sname"_pp"
...
# counter=0
# for args_f in $ARGS_PATH"/"$scenario_name/$sname-target*
# do
#     echo "found file" $args_f
#     echo "write data to" "$DUMP_DIR"${args_f#$ARGS_PATH}
#     $((counter++))

#     CUDA_VISIBLE_DEVIES=$1 python $controller @$args_f"/commandline_args.txt" --dir "$DUMP_DIR"${args_f#$ARGS_PATH}  --training_data_mode --height 256 --width 256 --gpu $1 --num 8 --seed $counter
#     break
# done
