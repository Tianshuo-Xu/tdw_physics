...
FULL_NAMES = [
    r.name for r in MODEL_LIBRARIES['models_full.json'].records if not r.do_not_use]
...
def get_args(dataset_dir: str, parse=True):
    ...
    parser = ArgumentParser(
        parents=[common], add_help=parse, fromfile_prefix_chars='@')
    ...
    def postprocess(args):
        ...
        if not (args.training_data_mode or args.readout_data_mode):
            ...
            PRIMITIVE_NAMES = [
                r.name for r in MODEL_LIBRARIES['models_flex.json'].records]
            global FULL_NAMES
            FULL_NAMES = [
                r.name for r in MODEL_LIBRARIES['models_full.json'].records]

        # choose a valid room
        assert args.room in (['box', 'tdw', 'house']
                             + ROOMS), (args.room, ROOMS)
        ...
        args.occluder_aspect_ratio = handle_random_transform_args(
            args.occluder_aspect_ratio)
        args.distractor_aspect_ratio = handle_random_transform_args(
            args.distractor_aspect_ratio)
        ...
        if args.distractor is None or args.distractor == 'full':
            ...
            args.distractor = [
                r.name for r in MODEL_LIBRARIES['models_core.json'].records]
        elif args.distractor in ['flex', 'primitives']:
            args.distractor = PRIMITIVE_NAMES
        else:
            d_names = args.distractor.split(',')
            args.distractor = [r for r in FULL_NAMES
                               + PRIMITIVE_NAMES if any((nm in r for nm in d_names))]
        ...
        if args.occluder is None or args.occluder == 'full':
            ...
            args.occluder = [r for r in FULL_NAMES
                             + PRIMITIVE_NAMES if any((nm in r for nm in o_names))]
        ...
        # seed MUST be pulled from a config.
            ...
            assert all((('seed' not in a)
                        for a in sys.argv[1:])), "You can't pass a new seed argument for generating the testing data; use the one in the commandline_args.txt config!"
...
class Dominoes(NoisyRigidbodiesDataset):
    """
    ...
    CUBE = [
        r for r in MODEL_LIBRARIES['models_flex.json'].records if 'cube' in r.name][0]
    PRINT = True
    ...
    def __init__(self,
                 ...
                 target_zone=['cube'],
                 # yellow is the default color for target zones
                 zone_color=[1.0, 1.0, 0.0],
                 zone_location=None,
    ...
    def get_types(self,
        ...
        if aspect_ratio_min:
            tlist = [r for r in tlist if self.aspect_ratios(
                r)[0] > aspect_ratio_min]
        if aspect_ratio_max:
            tlist = [r for r in tlist if self.aspect_ratios(
                r)[1] < aspect_ratio_max]
    ...
    def get_trial_initialization_commands(self) -> List[dict]:
        ...
        if self.probe_color is None:
            self.probe_color = self.target_color if (
                self.monochrome and self.match_probe_and_target_color) else None
    ...
    def _write_static_data(self, static_group: h5py.Group) -> None:
        ...
        if self.use_ramp:
            ...
            if self.ramp_base_height > 0.0:
                static_group.create_dataset(
                    "ramp_base_height", data=float(self.ramp_base_height))
                static_group.create_dataset(
                    "ramp_base_id", data=self.ramp_base_id)
        ...
        try:
            static_group.create_dataset(
                "target_rotation", data=xyz_to_arr(self.target_rotation))
        except (AttributeError, TypeError):
            pass
        try:
        ...
        try:
            static_group.create_dataset(
                "push_force", data=xyz_to_arr(self.push_force))
        except (AttributeError, TypeError):
            pass
        try:
            static_group.create_dataset(
                "push_position", data=xyz_to_arr(self.push_position))
        except (AttributeError, TypeError):
            pass
        try:
        ...
        try:
            static_group.create_dataset(
                "distractors", data=[r.name.encode('utf8') for r in self.distractors.values()])
        except (AttributeError, TypeError):
            pass
        try:
            static_group.create_dataset(
                "occluders", data=[r.name.encode('utf8') for r in self.occluders.values()])
        except (AttributeError, TypeError):
            pass
        ...
        try:
            static_group.create_dataset(
                "apply_force_to", data=self.apply_force_to.encode('utf8'))
        except (AttributeError, TypeError):
            pass

        try:
            static_group.create_dataset(
                "moving_name", data=self.moving_name.encode('utf8'))
        except (AttributeError, TypeError):
            pass
    ...
    def _update_target_position(self, resp: List[bytes], frame_num: int) -> None:
        if frame_num <= 0:
            ...
            try:
                ...
                print("Failed to get a new object position, %s" %
                      target_position_new)
    ...
    def _write_frame_labels(self,
        ...
        if has_target:
            ...
            has_moved = np.sqrt(
                (self.target_delta_position**2).sum()) > self.target_motion_thresh
            labels.create_dataset("target_delta_position",
                                  data=self.target_delta_position)
            ...
            labels.create_dataset("target_on_ground",
                                  data=self.target_on_ground)
        ...
        if has_target and has_zone:
            ...
            labels.create_dataset("target_contacting_zone",
                                  data=target_zone_contact)
    ...
    def rescale_record_to_size(record, size_range=1.0, randomize=False):
        ...
        if hasattr(size_range, 'keys'):
            ...
            scale = {}
            scale['x'] = Dominoes.rescale_record_to_size(
                record, size_range['x'], randomize)['x']
            scale['y'] = Dominoes.rescale_record_to_size(
                record, size_range['y'], randomize)['y']
            scale['z'] = Dominoes.rescale_record_to_size(
    ...
    def _place_target_object(self, size_range=None) -> List[dict]:
        ...
        if size_range is not None:
            scale = self.rescale_record_to_size(
                record, size_range, randomize=self.randomize_object_size)
        ...
        commands.extend(
            self.add_primitive(
                ...
                make_kinematic=True if (self._fixed_target and (
                    self.apply_force_to != 'target')) else False,
    ...
    def _place_and_push_probe_object(self, size_range=None) -> List[dict]:
        ...
        record, data = self.random_primitive(self._probe_types,
                                             ...
                                             exclude_color=(
                                                 self.target_color if exclude else None),
                                             exclude_range=0.25,
                                             add_data=False)
        o_id, scale, rgb = [data[k] for k in ["id", "scale", "color"]]
        ...
        if self.probe_horizontal:
            rot["z"] = 90
            self.probe_initial_position["z"] += - \
                np.sin(np.radians(rot["y"])) * scale["y"] * 0.5
            self.probe_initial_position["x"] += np.cos(
                np.radians(rot["y"])) * scale["y"] * 0.5

        if self.use_ramp:
            commands.extend(self._place_ramp_under_probe())

        if self.probe_has_friction:
            probe_physics_info = {'dynamic_friction': 0.1,
                                  'static_friction': 0.1, 'bounciness': 0.6}
        else:
            probe_physics_info = {'dynamic_friction': 0.01,
        ...
        commands.extend(
            self.add_primitive(
                ...
                o_id=o_id,
                add_data=True,
                make_kinematic=(False if self.apply_force_to
                                == 'probe' else True),
    ...
    def _place_ramp_under_probe(self) -> List[dict]:
        ...
        self.ramp_end_x = self.ramp_pos['x'] + \
            self.ramp_scale['x'] * r_len * 0.5
        ...
        self.probe_initial_position['y'] = self.ramp_scale['y'] * \
            r_height + self.ramp_base_height + self.probe_initial_position['y']
    ...
    def _add_ramp_base_to_ramp(self, color=None) -> None:
        ...
        self.colors = np.concatenate(
            [self.colors, np.array(color).reshape((1, 3))], axis=0)
    ...
    def get_record_dimensions(record: ModelRecord) -> List[float]:
        length = np.abs(record.bounds['left']
                        ['x'] - record.bounds['right']['x'])
        height = np.abs(record.bounds['top']
                        ['y'] - record.bounds['bottom']['y'])
        depth = np.abs(record.bounds['front']
                       ['z'] - record.bounds['back']['z'])
    ...
    def _set_avatar_attributes(self, avatar_position) -> None:
        ...
        self.camera_position = {
            k: a_pos[k] - self.room_center[k] for k in a_pos.keys()}
        self.camera_rotation = np.degrees(np.arctan2(
            self.camera_position['z'], self.camera_position['x']))
        dist = TDWUtils.get_distance(self.camera_position, self.camera_aim)
        self.camera_altitude = np.degrees(
            np.arcsin((self.camera_position['y'] - self.camera_aim['y'])/dist))
        camera_ray = np.array(
            [self.camera_position['x'], 0., self.camera_position['z']])
        ...
        opposite = np.array([-self.camera_position['x'],
                             0., -self.camera_position['z']])
    ...
    def _get_occluder_position_pose_scale(self, record, unit_position_vector):
        ...
        bounds = {k: np.maximum(np.abs(v), bounds[k])
                  for k, v in bounds_rot.items()}

        # make sure it's in a reasonable size range
        size = max(list(bounds.values()))
        size = np.minimum(np.maximum(
            size, self.occluder_min_size), self.occluder_max_size)
        ...
        if np.abs(pos['z']) < (self.occluder_min_z + self.occluder_min_size):
            pos.update(
                {'z': np.sign(pos['z']) * (self.occluder_min_z + self.occluder_min_size)})

        reach_z = np.abs(pos['z']) - 0.5 * bounds['z']
        if reach_z < self.occluder_min_z:  # scale down
            scale_z = (np.abs(pos['z'])
                       - self.occluder_min_z) / (0.5 * bounds['z'])
        else:
            scale_z = 1.0
        ...
        if self.num_occluders > 1 and len(self.occluder_positions):
            ...
            if (pos['x'] + self.occluder_min_size) > (last_pos_x - 0.5 * last_bounds_x):
                pos.update(
                    {'x': (last_pos_x - 0.5 * last_bounds_x) - self.occluder_min_size})

            reach_x = pos['x'] + 0.5 * bounds['x']
            if reach_x > (last_pos_x - 0.5 * last_bounds_x):  # scale down
                scale_x = (last_pos_x - 0.5 * last_bounds_x
                           - pos['x']) / (0.5 * bounds['x'])
            else:
                scale_x = 1.0
        ...
        if self.rescale_occluder_height:
            ...
            occ_target_height = self.camera_aim['y'] + \
                occ_dist * np.tan(np.radians(self.camera_altitude))
            occ_target_height *= self.occlusion_scale
            occ_target_height = np.minimum(
                occ_target_height, self.occluder_max_size)
    ...
    def _get_distractor_position_pose_scale(self, record, unit_position_vector):
        ...
        size = max(list(bounds.values()))
        size = np.minimum(np.maximum(
            size, self.distractor_min_size), self.distractor_max_size)
        scale = size / max(list(bounds.values()))
        bounds = self.scale_vector(bounds, scale)
        ...
        if np.abs(pos['z']) < (self.distractor_min_z + self.distractor_min_size):
            pos.update(
                {'z': np.sign(pos['z']) * (self.distractor_min_z + self.distractor_min_size)})

        reach_z = np.abs(pos['z']) - 0.5 * bounds['z']
        if reach_z < self.distractor_min_z:  # scale down
            scale_z = (np.abs(pos['z'])
                       - self.distractor_min_z) / (0.5 * bounds['z'])
        else:
            scale_z = 1.0
        bounds = self.scale_vector(bounds, scale_z)
        scale *= scale_z
        ...
        if self.num_distractors > 1 and len(self.distractor_positions):
            ...
            if (pos['x'] + self.distractor_min_size) > (last_pos_x - 0.5 * last_bounds_x):
                pos.update(
                    {'x': (last_pos_x - 0.5 * last_bounds_x) - self.distractor_min_size})
    ...
    def _place_background_distractors(self, z_pos_scale=4.) -> List[dict]:
        ...
        max_theta = self.distractor_angular_spacing * \
            (self.num_distractors - 1) * np.sign(opposite['z'])
        ...
        for i, o_id in enumerate(self.distractors.keys()):
            ...
            pos, rot, scale = self._get_distractor_position_pose_scale(
                record, pos_unit)
    ...
    def _place_occluders(self, z_pos_scale=4.0) -> List[dict]:
        ...
        for i, o_id in enumerate(self.occluders.keys()):
            ...
            # set a position
            theta = thetas[i]
            pos_unit = self.rotate_vector_parallel_to_floor(
                self.camera_ray, theta)

            pos, rot, scale = self._get_occluder_position_pose_scale(
                record, pos_unit)
...
class MultiDominoes(Dominoes):
    ...
    def __init__(self,
        ...
        self.spacing = self.collision_axis_length / \
            (self.num_middle_objects + 1.)
    ...
    def _write_static_data(self, static_group: h5py.Group) -> None:
        ...
        if self.middle_type is not None:
            static_group.create_dataset("middle_objects", data=[self.middle_type.encode(
                'utf8') for _ in range(self.num_middle_objects)])
            static_group.create_dataset("middle_type", data=self.middle_type)

    @staticmethod
    def get_controller_label_funcs(classname='MultiDominoes'):
        funcs = super(
            MultiDominoes, MultiDominoes).get_controller_label_funcs(classname)
