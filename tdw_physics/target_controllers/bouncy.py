...
from tdw_physics.util import MODEL_LIBRARIES, get_parser, none_or_str, int_or_bool
...
class Bouncy(Dominoes, FlexDataset):
    FLEX_RECORDS = ModelLibrarian(os.path.join(
        os.path.dirname(__file__), 'flex.json')).records
    CLOTH_RECORD = MODEL_LIBRARIES["models_special.json"].get_record(
        "cloth_square")
    ...
    RECEPTACLE_RECORD = MODEL_LIBRARIES["models_special.json"].get_record(
        "fluid_receptacle1x1")
    ...
    def __init__(self, port: int = 1071,
        ...
        if self.use_fluid:
            self.ft_selection = random.choice(
                self.FLUID_TYPES.fluid_type_names)
    ...
    def add_flex_solid_object(self,
        ...
        if add_data:
            self._add_name_scale_color(
                record, {'color': color, 'scale': scale, 'id': o_id})
    ...
    def drop_cloth(self) -> List[dict]:
        ...
        self.cloth_mass = 2
        self.cloth_data = {"name": self.cloth.name, "color": self.cloth_color,
                           "scale": self.cloth_scale, "id": self.cloth_id}
    ...
    def _get_zone_location(self, scale):
        # dratio = random.uniform(self.min_distance_ratio,
    ...
    def _build_intermediate_structure(self) -> List[dict]:
        ...
        # anchor object list
        # example:["cone","cube","cylinder","pyramid","triangular_prism"]
        anchor_list = ["cube"]

        # bounce inference object
        ...
        # bouncing platform
        self.objrec3 = MODEL_LIBRARIES["models_flex.json"].get_record(
            random.choice(["cube"]))
       ...
       # gate
        self.objrec4 = MODEL_LIBRARIES["models_flex.json"].get_record(
            random.choice(["triangular_prism"]))
        self.objrec4_id = self._get_next_object_id()
        self.objrec4_position = {'x': -0.2, 'y': 0.4, 'z': 0.}
        # {'x': 0, 'y': 0, 'z': 0},
        self.objrec4_rotation = {k: 0 for k in ['x', 'y', 'z']}
        self.objrec4_scale = {'x': 0.09, 'y': 0.3, 'z': 1.0}
        self.objrec4_mass = 500.0
        commands.extend(self.add_flex_solid_object(
            record=self.objrec4,
    ...
    def get_flex_object_collision(flex, obj1, obj2, collision_thresh=0.15):
        ...
        if (p1 is not None) and (p2 is not None):
            ...
            min_dist = dists.min()
            print(obj1, p1.shape, obj2, p2.shape, "min_dist",
                  min_dist, "colliding?", collision)
    ...
    def _write_frame_labels(self,
        ...
        labels, resp, grame_num, done = RigidbodiesDataset._write_frame_labels(
        ...
        if has_target and has_zone and (flex is not None):
            ...
            labels.create_dataset(
                "minimum_distance_target_to_zone", data=min_dist)
    ...
    def get_controller_label_funcs(classname='Bouncy'):
        ...
        funcs = super(Bouncy, Bouncy).get_controller_label_funcs(classname)
        funcs += get_all_label_funcs()

        def minimum_distance_target_to_zone(f):
            frames = list(f['frames'].keys())
            min_dists = np.stack([
                np.array(f['frames'][fr]['labels']
                         ['minimum_distance_target_to_zone'])
