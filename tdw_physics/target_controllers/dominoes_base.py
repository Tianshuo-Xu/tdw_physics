...
def get_args(dataset_dir: str, parse=True, args_file=None):
    """
    Combine Domino-specific arguments with controller-common arguments
    """
    #args_file = "/home/htung/Documents/2021/physics-benchmarking-neurips2021-htung/stimuli/generation/configs/dominoes_pp/pilot_dominoes_2distinct_1middle_tdwroom_fixedcam/commandline_args.txt"
    #args_string = open(args_file, "r").read()

    common = get_parser(dataset_dir, get_help=False)
    # if this is the main, use @
    # if this is not the main, specify args_file
...
class Dominoes(RigidbodiesDataset):
    ...
    def __init__(self,
        ...
        self.is_main_scene=is_main_scene
        if is_main_scene:
            ...
            self.secondscene = secondscene_fn(is_main_scene=False, args_file=secondscene_args)
        else:
            # set random state
            # import inherit from class Dataset
            randomize = 0
            seed = 0
            monochrome = False
        ...
        # things to pass to secondscene
        self.repeat_trial = None
        self.star_object = None
        self.candidate_dict = None
        self.normal_mass = None
        self.num_interactions = None
        self.num_distinct_objects = None
        self.save_passes = None
        self.write_passes = None
        self.save_movies = None
        self.save_meshes = None
        self.png_dir = None
        ...
        # make this scenario specific data
    ...
    def trial(self,
        ...
        # share object part, generate with parameter 1
        if object_info == None:
        ...
        ## clear static data always happen on the main function
        self.clear_static_data()
        if interact_id != self.num_interactions - 1 :
            get_trial_initialization_commands = self.get_trial_initialization_commands
            _get_object_meshes = self._get_object_meshes
            _write_frame_labels = self._write_frame_labels
            _set_segmentation_colors = self._set_segmentation_colors
            _write_frame = self._write_frame
            self._trial_num = trial_num

        else:
            self.secondscene.controller.clear_static_data()
            get_trial_initialization_commands = self.secondscene.controller.get_trial_initialization_commands
            _get_object_meshes = self.secondscene.controller._get_object_meshes
            _write_frame_labels = self.secondscene.controller._write_frame_labels
            _write_frame = self.secondscene.controller._write_frame
            _set_segmentation_colors = self.secondscene.controller._set_segmentation_colors
            self.secondscene.controller._trial_num = trial_num
            self.secondscene.controller.repeat_trial = self.repeat_trial
            self.secondscene.controller.star_object = self.star_object
            self.secondscene.controller.candidate_dict = self.candidate_dict
            self.secondscene.controller.normal_mass = self.normal_mass
            self.secondscene.controller.num_interactions = self.num_interactions
            self.secondscene.controller.num_distinct_objects = self.num_distinct_objects
            self.secondscene.controller.save_passes=self.save_passes
            self.secondscene.controller.write_passes=self.write_passes
            self.secondscene.controller.save_movies=self.save_movies
            self.secondscene.controller.save_meshes=self.save_meshes
            self.secondscene.controller.png_dir=self.png_dir


            # will create some data conter, id list and so
        ...
        commands.extend(get_trial_initialization_commands(interact_id))
        # Add commands to request output data.

        if interact_id != self.num_interactions - 1 :
            force_wait= self.force_wait
        else:
            force_wait = self.secondscene.controller.force_wait
        #resp = self.communicate(commands)

        commands.extend(self.add_curtain())
        commands.extend(self._get_send_data_commands()) # this is in trasnformed data
        ...
        _set_segmentation_colors(resp)

        _get_object_meshes(resp)
        frame = 0
        ...
        if interact_id > 0: #curtain move from center to the left
            ...
            for i in range(200):
                ...
                frame_grp, objs_grp, tr_dict, _ = _write_frame(frames_grp=frames_grp, resp=resp, frame_num=frame)

                # Write whether this frame completed the trial and any other trial-level data
                labels_grp, _, _, _ = _write_frame_labels(frame_grp, resp, frame, done)
            ...
            frame_grp, _, _, _ = _write_frame(frames_grp=frames_grp, resp=resp, frame_num=frame)
            _write_frame_labels(frame_grp, resp, -1, False)
        ...
        while not done:
            frame += 1
            # print('frame %d' % frame)
            resp = self.communicate(self.get_per_frame_commands(resp, frame, force_wait=force_wait+before_start_frame))
            ...
            frame_grp, objs_grp, tr_dict, done = _write_frame(frames_grp=frames_grp, resp=resp, frame_num=frame)

            # Write whether this frame completed the trial and any other trial-level data
            labels_grp, _, _, done = _write_frame_labels(frame_grp, resp, frame, done)
    ...
    def _place_target_zone(self) -> List[dict]:
        ...
        # sample if you are just initilaizing the main scenario or
        # initializing the second_scene
        if not self.repeat_trial or not self.is_main_scene: # sample from scratch
...
class MultiDominoes(Dominoes):
    ...
    def __init__(self,
        ...
        super().__init__(port=port, is_main_scene=is_main_scene, secondscene_fn=secondscene_fn, secondscene_args=secondscene_args,**kwargs)
...
class DominoesScenario():
    def __init__(self, is_main_scene=True, secondscene_fn=None,args_file=None):
        """
        if is_main_scene=True and you wish to have a second scenario, specify your secondscene_fn,
        and make sure you have specify argument for it with --secondscene_args

        if is_main_scene=False, you need specify args_file to initiate the class
        """


        if not is_main_scene:
            assert(args_file is not None)
            args = get_args("dominoes", args_file=args_file)
        else:
            args = get_args("dominoes")

        DomC = MultiDominoes(
            is_main_scene=is_main_scene,
            secondscene_fn=secondscene_fn,
            secondscene_args=args.secondscene_args,
            port=args.port,
            room=args.room,
