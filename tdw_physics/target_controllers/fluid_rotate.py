...
class Fluid(Dominoes, FlexDataset):
    """
    ...
    def __init__(self,
        ...
        if self.use_fluid:
            #self.ft_selection =[key for key in self.FLUID_TYPES][0]
            self.ft_selection =random.choice([key for key in self.FLUID_TYPES])
    ...
    def set_target_types(self, olist):
        ...
        clist = ['fluid_receptacle1x1']
        self._container_types = [r for r in MODEL_LIBRARIES['models_special.json'].records if r.name in clist]

        vlist = ['prim_cube', 'prim_sphere', 'prim_cyl', 'fluid_receptacle1x1']#, 'prim_capsule']#, 'fluid_receptacle1x2'] #'prim_cube',
        #import ipdb; ipdb.set_trace()
        #, 'prim_cube', 'cloth_1meter_square', 'prim_cyl', 'prim_sphere',
        #         'fluid_receptacle1m_round', 'fluid_receptacl1x1', 'fluid_receptacle1x2',
        #          'prim_capsule']
        self._target_types = [r for r in MODEL_LIBRARIES['models_special.json'].records if r.name in vlist]
    ...
    def get_scene_initialization_commands(self) -> List[dict]:
        ...
        if self.use_fluid:
            create_container.update({
                'viscosity': self.FLUID_TYPES[self.ft_selection].viscosity,
                'adhesion': self.FLUID_TYPES[self.ft_selection].adhesion,
                'cohesion': self.FLUID_TYPES[self.ft_selection].cohesion,
                'fluid_rest': self.particle_spacing,
                'damping': 0.01,
                'subsetp_count': 5,
                'iteration_count': 8,
                'buoyancy': 1.0})
        ...
        # fluid_id = 2
        # fluid_name='water'
        # water_commands = [{"$type": "load_flex_fluid_from_resources",
        #                    "id": fluid_id,
        ...
        #commands.extend(water_commands)
    ...
    def get_per_frame_commands(self, resp: List[bytes], frame: int) -> List[dict]:
        ...
        rotate_cmd = {"$type": "rotate_object_by", "axis": "roll", "angle": 1.25, "id": self.container_id}
        self.rotate_duration = 100
       # import ipdb; ipdb.set_trace()
        if frame < self.rotate_duration:
            if self.PRINT:
                print("applied %s at time step %d" % (rotate_cmd, frame))
            return [rotate_cmd]
        else:
            print(frame)
            return []
    ...
    def _place_intermediate_object(self) -> List[dict]:
        ...
        print("=========")
        print("intermefaite object", record.name, data["scale"])
        ...
        if "xxxxreceptacle" in record.name:
            ...
            if "receptacle" in record.name:
                scale['x'] = 1.0
                scale['y'] = 1.0
                scale['z'] = 1.0
            commands.extend(
                self.add_flex_solid_object(
                    record=record,
                    ...
                    scale=scale,
                    add_data = False,
                    #dynamic_friction=0.1, #increased friction
                    #static_friction=0.1,
                    #bounciness=0,
                    o_id=o_id))
    ...
    def _place_drop_object(self) -> List[dict]:
        ...
        self.fluid_color = self.target_color if self.target_color is not None else self.random_color()
        self.fluid_scale = {'x': 1.0, 'y': 1.0, 'z': 1.0}
        commands = []
        ...
        self._add_name_scale_color(
            self.ft_selection, {'color': self.fluid_color, 'scale': self.fluid_scale, 'id': self.fluid_id}, use_name=True)
        self.drop_position = {
        ...
        print("container object", record.name, data["scale"])
        ...
        self.container_id = o_id
        self.non_flex_objects.append(self.container_id)

        if self.container_rotation is None:
            self.container_rotation = self.get_rotation(self.target_rotation_range)
        ...
        tmp = self.add_flex_solid_object(
                record=record,
    ...
    def _place_target_zone(self) -> List[dict]:
        ...
        self._zone_types = [r for r in MODEL_LIBRARIES['models_special.json'].records if r.name=="prim_cube"]
        ...
        container_record = [r for r in MODEL_LIBRARIES['models_special.json'].records if r.name=="fluid_receptacle1x1"][0]

        #import ipdb; ipdb.set_trace()
        # commands.extend(self.add_physics_object(record=container_record,
        #                   o_id=receptacle_id,
        #                   position={"x": -0.35, "y": 0, "z": 0},
        #                   rotation={"x": 0, "y": 0, "z": 0},
        #                   dynamic_friction=0.1,
