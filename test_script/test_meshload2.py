...
exclude_list = ["12_01_015", "am151_043_00", "axe_lowpoly", "b03_bus-sum_wom_rtsit_716", "b03_pen_4colours",\
                "b03_vm_hg2_025_umbrella", "b04_croissant", "b04_horse_body_mesh", "b04_polysurface2", "b05_barazza_ovens_60_1fvltim"\
                "b05_donutz", "b05_gym_matrix_t7xi_treadmill", "b05_radiator", "b05_woman_boot", "b06_firehydrant_lod0",\
                "bike_forma", "ceramic_pot", "fire_extinguisher", "jaipur_4", "lunar_globe_art_auctor", "nature_stone_040",\
                "roland_rg-1f", "rucksack", "star_trac_treadmill", "vm_v5_024"]

c = Controller()
obj_list = lib.records[2335:]
obj_list = [record for record in lib.records if record.name=="shar_pei"]

for record in tqdm(obj_list):
    ...
    if not record.flex or record.do_not_use:
        continue
    ...
    resp = c.communicate({"$type": "send_meshes", "frequency": "once"})
    ...
    if vertices.shape[0] == 0:
        ...
        outf = open("models_full_check.txt", "a")
        outf.write(",".join([obj_name] + ["0"] * 3) + "\n")
        outf.close()
        #import ipdb; ipdb.set_trace()
    else:
        record_bounds = np.array([abs(record.bounds["right"]["x"] - record.bounds["left"]["x"]), abs(record.bounds["top"]["y"] - record.bounds["bottom"]["y"]), abs(record.bounds["front"]["z"] - record.bounds["back"]["z"])])
        mesh_bounds = np.max(vertices, 0) - np.min(vertices, 0)
        print("bounds from record", record.bounds["right"]["x"] - record.bounds["left"]["x"], record.bounds["top"]["y"] - record.bounds["bottom"]["y"], record.bounds["front"]["z"] - record.bounds["back"]["z"])
        print("bounds from mesh", np.max(vertices, 0) - np.min(vertices, 0))

        ratio = record_bounds/mesh_bounds


        print(ratio)
        close_to_ones = abs(ratio - 1) < 0.0001
        if close_to_ones[0] * close_to_ones[1] * close_to_ones[2]:
            print("good mesh")
        else:
            print("bad mesh")
            #import ipdb; ipdb.set_trace()
            outf = open("models_full_check.txt", "a")
            outf.write(",".join([obj_name] + [str(ratio[i]) for i in range(3)]) + "\n")
            outf.close()
